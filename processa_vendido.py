...
def _ler_arquivo_vendido(caminho_arquivo: Path, cabecalho: list[str]) -> pd.DataFrame:
    """Le um TXT de vendido e devolve um DataFrame tratado."""
    ...
    df = df.rename(columns={"codigo": "cod_rca"})
...
def consolidar_vendidos(
    ...
    """
    Le todos os arquivos da pasta `Vendido`, trata os dados e concatena.

    Parameters
    ----------
    pasta_vendido:
        Pasta com os arquivos .txt por fornecedor.
    salvar_em:
        Se informado, salva o consolidado em .xlsx ou .csv.
    cabecalho:
        Lista de nomes das colunas. Se nao for informada, usa um padrao provisÃ³rio
        baseado na estrutura atual dos arquivos.
    """
